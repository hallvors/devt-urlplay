	...
	if(this.consoleClient) {
		...
		var p = new Promise((function(resolve, reject) {
			...
			this.consoleClient.evaluateJSAsync(script, function(result) {
				...
				if(result.exception) {
					if(typeof result.exception === "object" && result.exception.preview) {
						realResult = result.exception.preview; // TODO: do we need more data than the preview contains?
					} else {
						// Gecko's internal APIs are so inconsistent :-/ Throw something and there's no stack
						// all you get is what the script threw. We'll make something up..
						realResult = {kind:"Exception", name:"CustomException", message: result.exception};
					}
				} else {
					try{
						...
						console.log("realResult", JSON.stringify(realResult, null, 2));
